...
// Update: same validation as create but without membership (only when status is under_review)
const updateApplicationValidation = [
  ...
  body('nominee.relationship').trim().notEmpty().withMessage('Nominee relationship is required'),
  body('nominee.mobileNumber').trim().notEmpty().withMessage('Nominee mobile number is required').matches(/^\d{10}$/).withMessage('Nominee mobile number must be 10 digits'),
  body('nominee.bankAccountNumber').optional().trim(),
  body('nominee.address.village').trim().notEmpty().withMessage('Nominee village is required'),
  body('nominee.address.postOffice').trim().notEmpty().withMessage('Nominee post office is required'),
  body('nominee.address.policeStation').trim().notEmpty().withMessage('Nominee police station is required'),
  body('nominee.address.district').trim().notEmpty().withMessage('Nominee district is required'),
  body('nominee.address.pinCode').matches(/^\d{6}$/).withMessage('Nominee PIN code must be 6 digits'),
  body('guarantor.name').trim().notEmpty().withMessage('Guarantor name is required'),
  body('guarantor.fatherOrHusbandName').trim().notEmpty().withMessage('Guarantor father\'s/husband\'s name is required'),
  body('guarantor.relationship').trim().notEmpty().withMessage('Guarantor relationship is required'),
  body('guarantor.mobileNumber').trim().notEmpty().withMessage('Guarantor mobile number is required').matches(/^\d{10}$/).withMessage('Guarantor mobile number must be 10 digits'),
  body('guarantor.bankAccountNumber').optional().trim(),
  body('guarantor.address.village').trim().notEmpty().withMessage('Guarantor village is required'),
  body('guarantor.address.postOffice').trim().notEmpty().withMessage('Guarantor post office is required'),
  body('guarantor.address.policeStation').trim().notEmpty().withMessage('Guarantor police station is required'),
  body('guarantor.address.district').trim().notEmpty().withMessage('Guarantor district is required'),
  body('guarantor.address.pinCode').matches(/^\d{6}$/).withMessage('Guarantor PIN code must be 6 digits'),
  body('coApplicant')
    .optional()
    .custom((value) => {
      if (value !== null && value !== undefined && typeof value !== 'object') {
