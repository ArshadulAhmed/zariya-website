...
// @desc    Update loan - Admin only for active loans, Admin/Employee for pending/rejected
  ...
  try {
    ...
    if (status) {
      ...
      if (status === 'closed' && req.user.role === 'admin') {
        // Verify loan is active and fully paid
    ...
    // Prevent modifying critical fields if loan is active
    if (loan.status === 'active') {
