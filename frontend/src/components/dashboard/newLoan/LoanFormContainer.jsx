...
import { createApplication, updateApplication } from '../../../store/slices/loanApplicationsSlice'
...
const buildApplicationPayload = (formData, hasCoApplicant, selectedMembership) => {
  ...
  loanData.coApplicant = hasCoApplicant
    ? {
...
const LoanFormContainer = ({ mode = 'create', applicationId = null }) => {
  ...
  const detailPath = applicationId ? `/dashboard/loan-applications/${applicationId}` : '/dashboard/loan-applications'
  ...
  const handleSubmit = async (e) => {
    ...
    const loanData = buildApplicationPayload(formData, hasCoApplicant, selectedMembership)

    if (isEditMode && applicationId) {
      const result = await dispatch(updateApplication({ id: applicationId, applicationData: loanData }))
      if (updateApplication.fulfilled.match(result)) {
        navigate(detailPath)
      }
    } else {
      const payload = { ...loanData, membership: selectedMembership._id || selectedMembership.id }
      ...
      <div className="form-footer">
        <div className="form-actions">
          <button
            ...
            onClick={() => navigate(isEditMode ? detailPath : '/dashboard/loans')}
          >
            Cancel
          </button>
          <button
            ...
            disabled={isLoading}
          >
            {isLoading ? (isEditMode ? 'Saving...' : 'Submitting...') : (isEditMode ? 'Save changes' : 'Submit Loan Application')}
          </button>
        </div>
      </div>
    </form>
